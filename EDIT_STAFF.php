...
else
{
 ...
 if(isset($_POST['update']))
  ...
  $data = getPosts();
    ...
    $update_Query = "UPDATE `staff_reg` SET `staff_name`='$data[2]',`status`='$data[3]',`dept`='$data[4]',`email`='$data[5]',`phone`='$data[6]' WHERE `staff_id` = $data[1]";
    try{
...
<input type="submit" name="update" value="UPDATE" onclick="return confirm('Are you sure you want to Update this record?')" style="height:50px; width:150px" /></a>
  </div>
